...
lessThan(QT_MAJOR_VERSION, 6) {
    error("s2t-qt-client cần Qt 6 (đang dùng Qt $$QT_VERSION). Trên RHEL 9 hãy gọi qmake6, không phải qmake.")
}
!versionAtLeast(QT_VERSION, 6.2.0) {
    error("s2t-qt-client cần Qt 6.2 trở lên (đang dùng Qt $$QT_VERSION).")
}
!qtHaveModule(multimedia) {
    error("Thiếu module Qt Multimedia. RHEL 9: sudo dnf install qt6-qtmultimedia-devel")
}

# Bộ mã proto3, HPACK, HTTP/2 và nhật ký dùng chung với s2t-qt-server.
include($$PWD/../shared/shared.pri)

# Đường dẫn include gốc.  shared/ đứng trước để "core/Logger.h" luôn tìm thấy
# bản dùng chung, còn "core/AppConfig.h" - chỉ có ở đây - rơi xuống $$PWD.
INCLUDEPATH += $$PWD
...
# only picks the starting position - see shared/core/Logger.h.
develop {
