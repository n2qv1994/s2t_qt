...
int main(int argc, char *argv[])
{
    ...
    // Still "s2t_qt", not "s2t-qt-client", even though the binary was renamed
    // when the project was split.  QStandardPaths and QSettings both derive
    // their location from this name: changing it would move
    // HKCU\Software\s2t\s2t_qt (and ~/.config/s2t/s2t_qt.conf) out from under
    // every deployed workstation, losing its server address and token without
