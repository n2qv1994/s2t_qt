...
public:
    ...
    // Whether the *buffer* can reach the inference tier.  Deliberately a
    // separate flag from connected(): a reachable buffer in front of a down
    // pipeline is a working state - audio is still being accepted and queued -
    // and collapsing the two into one light would send an operator to fix the
    // wrong machine.
    bool upstreamReady() const { return m_upstreamReady; }
