...
void SessionController::refreshConnection()
{
    ...
    m_rpc->call<asr::ModelStatusResponse>(
        this,
        [](AsrClient &client, asr::ModelStatusResponse &out) {
            // A real RPC, not just a TCP connect: "connected" has to mean the
            // adapter answers and the token is accepted, otherwise the badge
            // goes green against a server that rejects everything.
            return client.getModelStatus(&out, 5000);
        },
        [this](const grpc::Status &status, const asr::ModelStatusResponse &) {
            m_connectionPending = false;
            const bool wasConnected = m_connected;
            m_connected = status.ok();
            m_connectionDetail = status.ok() ? m_config->serverTarget : status.toString();
            // Probed every 3 s; only a change is worth a line, or the log is
            // 20 identical entries per minute.
            if (wasConnected != m_connected) {
                if (m_connected)
                    LOG_INFO(applog::cat::Session) << "AI server connected:" << m_connectionDetail;
                else
                    LOG_WARN(applog::cat::Session) << "AI server unreachable:" << m_connectionDetail;
            }
