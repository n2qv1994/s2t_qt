...
public:
    ...
    // come back UNIMPLEMENTED, which is exactly the right answer and is how
    // the client tells the two kinds of endpoint apart.
    grpc::Status bufferPing(const buf::PingRequest &req, buf::PingResponse *out, int timeoutMs);
    grpc::Status getBufferStatus(const buf::BufferStatusRequest &req,
                                 buf::BufferStatusResponse *out, int timeoutMs);
    grpc::Status listBufferedSessions(const buf::BufferSessionsRequest &req,
