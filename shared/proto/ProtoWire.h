...
public:
    ...
    // Consumes whatever is left without inspecting it.  Only useful for the
    // request messages that carry no fields at all: a handler still wants to
    // call parse() uniformly, and leaving the reader short of the end would
    // read as a decode failure to the caller that checks ok() afterwards.
    void skipRemaining() { m_p = m_end; }
